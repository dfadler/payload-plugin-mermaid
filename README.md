...
`mermaidBlock(overrides?)` takes a `Partial<Block>` to override any top-level
block config (e.g. `labels`) while keeping the default fields.
...
import type { MermaidBlock } from '../payload-types.js' // generated by your own project
import type { SerializedBlockNode } from '@payloadcms/richtext-lexical'
import type { JSXConverter } from '@payloadcms/richtext-lexical/react'

const mermaidConverter: JSXConverter<SerializedBlockNode<MermaidBlock>> = ({
  node,
}) => <Mermaid diagram={node.fields.diagram} caption={node.fields.caption} />
```

(`MermaidBlock` isn't exported by this package — the block's
`interfaceName: 'MermaidBlock'` tells Payload to generate that type into
_your own_ `payload-types.ts` the normal way, via `payload generate:types`,
once the block is registered in your config.)
